export default function Services() {
    return (
        <section className="services" id="produtos">
            <h2>Nossos Serviços</h2>
            <div className="services-container">
                <div className="service-card">
                    <img src="imgs/place_holder_400x400.png" alt="Serviço 1" />
                    <h3>Etiquetas de Balança Térmica</h3>
                    <p>Ideais para supermercados e padarias, garantem impressões nítidas e duradouras.</p>
                </div>
                <div className="service-card">
                    <img src="imgs/place_holder_400x400.png" alt="Serviço 2" />
                    <h3>Etiquetas Frágil para Entrega</h3>
                    <p>Sinaliza pacotes delicados, garantindo cuidado no transporte.</p>
                </div>
                <div className="service-card">
                    <img src="imgs/place_holder_400x400.png" alt="Serviço 3" />
                    <h3>Ribbons para Impressão</h3>
                    <p>Alta qualidade para impressoras térmicas, oferecendo durabilidade e acabamento profissional.</p>
                </div>
            </div>
        </section>
    );
}
